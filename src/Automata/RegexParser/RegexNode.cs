...
namespace System.Text.RegularExpressions {
    ...
    internal sealed class RegexNode {
        ...
        // complement (~). These kinds are above the upstream range and only ever
        // produced by the patched parser, so legacy .NET regex trees never contain them.
        internal const int Intersect  = 35;                   // n        A&B&C  (BREX &)
        internal const int Complement = 36;                   // 1        ~A     (BREX ~)
